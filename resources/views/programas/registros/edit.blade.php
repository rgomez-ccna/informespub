...
@php
    ...
    $tipoActual = old('tipo_fila', $registro->tipo_fila);

    if ($tipoActual !== 'normal') {
        $tipoActual = 'especial';
    }

    function valorCampoActual($campo, $valor) {
        if (!$valor) {
            return null;
        }

        return match ($campo->tipo) {
            'numero' => $valor->valor_numero,
            'fecha' => optional($valor->valor_fecha)->format('Y-m-d'),
            'hora' => $valor->valor_hora,
            'checkbox' => $valor->valor_texto,
            default => $valor->valor_texto,
        };
    }
@endphp
...
<div class="container py-4" style="max-width: 760px;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h4 class="mb-1 fw-bold text-dark">Editar fila</h4>
            ...
            <form action="{{ route('programas.bloques.registros.update', [$programa, $bloque, $registro]) }}" method="POST">
                ...
                <div id="bloque_especial" style="display:none;">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="texto_especial"
                                  ...
                                  rows="3"
                                  placeholder="Ej: Asamblea de Circuito: BUSQUEN LA PAZ">{{ old('texto_especial', $registro->texto_especial) }}</textarea>
                ...
                <div id="bloque_campos">
                    <div class="alert alert-light border py-2 small mb-3">
                        Editá los datos en el mismo orden en que aparecen en la tabla.
                    </div>

                    @foreach($programa->campos as $campo)
                        @php
                            $valor = $valores->get($campo->id);
                            $valorActual = old('campos.' . $campo->id, valorCampoActual($campo, $valor));
                        @endphp
